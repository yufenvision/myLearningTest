...
public class argsTestUpdate {
    ...
    private ArgsUpdate argsUpdate = new ArgsUpdate("-l -p 8080 -d /usr/logs -g this,is,a,list -D 1,2,-3,5");

    @Test
    public void in_l_out_boolen_true(){
        assertEquals(true, argsUpdate.getValue("l"));
    }

    @Test
    public void in_p_out_String_8080(){
        assertEquals(new Integer(8080), argsUpdate.getValue("p"));
    }

    @Test
    public void in_d_out_String(){
        assertEquals("/usr/logs", argsUpdate.getValue("d"));
    }

    @Test
    public void in_g_out_listString(){
        List<String> v = (List<String>) argsUpdate.getValue("g");
        assertEquals("this", v.get(0));
        assertEquals("is", v.get(1));
        assertEquals("a", v.get(2));
        assertEquals("list", v.get(3));

    }

    @Test
    public void in_g_out_listInteger(){
        List<Integer> v = (List<Integer>) argsUpdate.getValue("D");
        assertEquals(Integer.valueOf(1), v.get(0));
        assertEquals(Integer.valueOf(2), v.get(1));
        assertEquals(Integer.valueOf(-3), v.get(2));
        assertEquals(Integer.valueOf(5), v.get(3));
    }
...
//            }
//        });
        argsUpdate.splitNeedParseStr().forEach((k,v) -> System.out.println(k + " : " + v));
    }
}
